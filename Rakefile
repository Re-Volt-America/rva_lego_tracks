...
task :package do
  zipfile_name = "#{RVALegoTracks::NAME}.zip"
...
task version: [:package] do
  checksum = nil
  checksum = Digest::SHA256.hexdigest File.read "#{RVALegoTracks::NAME}.zip" if File.exist?("#{RVALegoTracks::NAME}.zip")

  contents = [
    '{',
    "\t\"rva_lego_tracks\":",
    "\t{",
    "\t\t\"description\": \"#{RVALegoTracks::DESCRIPTION}\",",
    "\t\t\"version\": #{RVALegoTracks::VERSION},",
    "\t\t\"checksum\": \"#{checksum}\",",
    "\t\t\"url\": \"#{RVALegoTracks::URL}\"",
    "\t}",
    '}'
  ]
